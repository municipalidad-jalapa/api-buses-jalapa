...
class CatalogoVehiculosIT extends IntegracionPostgisTest {
    ...
    void municipalidad_consulta_catalogo_y_administra_vehiculos_sin_emitir_credenciales() throws Exception {
        mockMvc.perform(get("/api/v1/admin/catalogo/vehiculos"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[*].identificador", hasItems("BUS-01", "BUS-02")))
                .andExpect(jsonPath("$[0].id").isNumber());
        mockMvc.perform(get("/api/v1/admin/vehiculos")).andExpect(status().isOk());
        mockMvc.perform(post("/api/v1/admin/vehiculos/1/equipos")).andExpect(status().isForbidden());
    }
